...
class Solution:
    def deleteDuplicates(self, head: ListNode) -> ListNode:
        if not head or not head.next:
            return head
        dummy = ListNode(next=head) #哑巴结点，链表头结点可能被删除，所以用 dummy node 辅助删除
        slow = dummy  #head前一个node
        fast = dummy.next #head
        while fast: #slow为主要的，fast存在，slow一定存在 
            if  fast.next and fast.next.val == fast.val:
                tmp = fast.val
                while fast and tmp == fast.val:
                    fast = fast.next
            else:
                slow.next = fast #将slow和fast节点连接起来
                slow = fast
                fast = fast.next # slow和fast同时推后
        slow.next = fast
